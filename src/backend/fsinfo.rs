...
pub fn read(path: &Path) -> Option<Info> {
    ...
    Some(Info {
        name: name_for(buf.f_type),
        free: buf.f_bavail.saturating_mul(buf.f_bsize.max(0) as u64),
    })
}
...
mod tests {
    ...
    fn a_known_magic_reads_back_as_its_name_and_an_unknown_one_as_hex() {
        ...
        assert_eq!(
            name_for(0x1234),
            "0x1234",
            "a wrong name would be worse than the number"
        );
    }
    ...
    fn a_real_directory_answers_a_name_and_a_nonzero_free() {
        ...
        assert!(
            info.free > 0,
            "a filesystem with no room left could not have held this sandbox"
        );
    }
    ...
    fn the_line_carries_the_name_and_the_free_bytes() {
        let line = fsinfo_line(&Some(Info {
            name: "btrfs".to_string(),
            free: 442_000_000_000,
        }));
